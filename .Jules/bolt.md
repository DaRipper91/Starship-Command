...
**Learning:** Components subscribed to the entire state object (`state.currentTheme`) re-render on _any_ update, even if only metadata changes. This causes expensive re-computations (like `parseFormatString`) unnecessarily.
