...
- **Imports**: Imports are automatically sorted. Run `pnpm run lint -- --fix` to sort them.
...
- `pnpm run format` (to fix formatting)
- `pnpm run lint` (to check for violations)
- `pnpm run build` (to ensure type safety - uses `tsc -b`)

We use `husky` and `lint-staged` to automatically check your code on commit. Running `pnpm run lint` is required to ensure consistent code styling.
