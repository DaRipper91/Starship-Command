...
function SortableItem({
  ...
  return (
    <div
      ...
      className={cn(
        "group flex items-center gap-3 rounded-md border p-3 shadow-sm transition-colors cursor-pointer",
        isSelected
          ? "border-blue-500 bg-gray-800 ring-1 ring-blue-500"
          : "border-gray-700 bg-gray-800 hover:border-gray-600 hover:bg-gray-750",
        isDragging && "opacity-50 ring-2 ring-blue-500 z-50 bg-gray-700"
      )}
    >
      <button
        {...listeners}
        {...attributes}
        className="cursor-grab text-gray-500 hover:text-gray-300 active:cursor-grabbing focus:outline-none"
...
export function ModuleList({ className }: { className?: string }) {
  const { currentTheme, updateConfig, selectedModule, setSelectedModule } = useThemeStore();
  ...
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      ...
      >
        ...
        <DragOverlay>
          {activeItem ? (
            <div className="group flex items-center gap-3 rounded-md border border-blue-500 bg-gray-700 p-3 shadow-lg opacity-90">
